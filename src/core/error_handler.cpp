...
namespace Lumi::ErrorHandler {
    ...
    void LUMI_LOG(std::string_view message, LogCode code, LogSeverity severity,
                  std::string_view expected, std::string_view actual, AssertType assertType) {
    ...
    bool LUMI_ASSERT(bool actualValue, bool expectedValue, const AssertType type) {
        ...
        LUMI_LOG("Assertion failed", LogCode::AssertionFailed, LogSeverity::Error,
                 std::to_string(expectedValue), std::to_string(actualValue), type);
        return true;
    }
